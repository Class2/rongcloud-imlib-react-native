...
var _rongIMConnectionStatus=function(resp){

}
DeviceEventEmitter.addListener('rongIMConnectionStatus',resp=>{
    typeof _rongIMConnectionStatus==='function' && _rongIMConnectionStatus(resp);
});
...
export default {
    ...
    onChanged (callback){
        _rongIMConnectionStatus = callback;
    },
    ...
    getCurrentConnectionStatus(){
        return RongCloudIMLib.getCurrentConnectionStatus();
    },
    //获得会话中的聊天信息
    ...
    searchMessages(conversationType, targetId,keyword, count,beginTime){
        return RongCloudIMLib.searchMessages(conversationType, targetId,keyword, count,beginTime);
    ...
    sendTextMessage (conversationType, targetId, content) {
      if(Platform.OS=="ios"){
          ...
          return RongCloudIMLib.sendTextMessage(conversationType, targetId, content, content,content);
      }
